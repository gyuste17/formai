...
```mermaid
pie title Distribución del Tiempo Administrativo Sin IA
    "Tareas manuales repetitivas (emails, PDFs, datos)" : 45
    "Gestión de hojas de cálculo y reportes" : 25
    "Atención directa a clientes y proveedores" : 20
    "Tareas estratégicas y mejora de procesos" : 10
```
...
```mermaid
flowchart LR
    A[Herramientas Diarias de Administración] --> B[Microsoft Copilot en Word, Excel y Outlook]
    A --> C[ChatGPT y Claude para Análisis y Redacción]
    A --> D[Power Automate para Flujos sin Código]
    
    B --> E[Borradores de emails y actas de reuniones en Teams en 30s]
    C --> F[Extracción de tablas y resúmenes de contratos PDF extensos]
    D --> G[Guardado automático de adjuntos y avisos de cobro]
```
