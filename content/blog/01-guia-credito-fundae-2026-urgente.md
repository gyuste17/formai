...
```mermaid
flowchart LR
    A[Cotización mensual Seg. Social 0.7%] --> B[Bolsa de Crédito Anual FUNDAE]
    B -->|Se ejecuta formación antes del 31 Dic| C[Bonificación directa en TC1 / RLC = Coste 0€]
    B -->|No se utiliza antes del 31 Dic| D[Pérdida definitiva del crédito no consumido]
```
...
```mermaid
timeline
    title Plazos obligatorios de gestión FUNDAE
    Día -7 o -2 : Comunicación de Inicio en la plataforma telemática FUNDAE (mínimo 2 días antes si no hay RLT, o con informe RLT)
    Durante el curso : Registro de asistencia y conexión activa superior al 75% de las horas
    Día +30 máx : Finalización y cuestionarios de evaluación de calidad
    Mes siguiente : Notificación en Seguridad Social (casilla de bonificación por formación continua en el RLC)
```
